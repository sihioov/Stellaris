...
async fn main() -> dysonsphere::error::Result<()> {
    ...
    match source.as_str() {
        "file" => {
            let path = std::env::var("LANIAKEA_FILE_PATH").unwrap_or_else(|_| "tasks.json".into());
        ...
        "rabbitmq" => {
            ...
            let topic = std::env::var("LANIAKEA_RABBITMQ_TOPIC").unwrap_or_else(|_| "tasks".into());
            log::info!("Starting rabbitmq mode (uri={uri}, topic={topic})");
            log::warn!(
                "RabbitMQ mode: ack-on-success (at-least-once at worker boundary). Status not persisted. MVP only."
            );
