...
use dysonsphere::error::{StellarisError, Result};
...
impl RabbitMQTaskDispatcher {
    ...
    pub fn set_worker_status(&mut self, instance_name: &str, status: WorkerStatus) -> Result<()> {
        if let Some(worker) = self.worker_queues.get_mut(instance_name) {
            ...
            Err(StellarisError::NotFound(format!("Worker {} not found", instance_name)))
        }
    }
    ...
    fn round_robin(&self, workers: &[String]) -> Result<String> {
        ...
        let index = self.current_worker_index.fetch_add(1, std::sync::atomic::Ordering::SeqCst) % workers.len();
    ...
    fn default_round_robin(&self) -> Result<String> {
        ...
        if worker_count == 0 {
            return Err(StellarisError::MQError("No active workers available".to_string()));
        }
        
        // 활성 워커 목록 생성
        let active_workers: Vec<String> = self.worker_queues.iter()
